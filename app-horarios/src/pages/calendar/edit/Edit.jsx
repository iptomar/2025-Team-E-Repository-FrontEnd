...
export default function CalendarEdit() {
    ...
    const fetchAll = async () => {
      try {
        ...
        const transformedCourses = subjects.map((subject, index) => {
          const course = {
            id: subject.Id,
            subjectId: subject.IdSubject, // ✅ Use subject.Id instead of subject.IdSubject
            name: subject.Subject,
            professorId: subject.professorId,
            professor: subject.Professor,
            type: subject.Tipologia,
            color: tipologyColor(subject.Tipologia),
          };
        ...
        console.log("Schedule blocks", scheduleData);
        const evts =
          scheduleData.blocks?.map((b) => {
            ...
            return {
              id: b.Id,
              title: `${subj?.Subject || b.SubjectName} - ${
  ...
  const handleDateSelect = (selectInfo) => {
    ...
    if (hasOverlap) {
      setMessage({ text: "Já existe uma aula nesse horário", type: "danger" });
      selectInfo.view.calendar.unselect();
      return;
    }
    ...
    console.log("selected eventsss", newEvent);
  ...
  const handleEventClick = (clickInfo) => {
    ...
    console.log("already event");
    setSelectedEvent(event);
    setSelectedRoom(
      event.extendedProps.room || event.extendedProps.classroomId || ""
    );
  ...
  const handleRoomAssign = async () => {
    ...
    console.log("selectedeventblock", selectedEvent.extendedProps.blockId )
    ...
    socket.once("respostaConflitosSala", async ({ blocos }) => {
      ...
      const conflitos = blocos.map((bloco, idx) => {
        ...
        console.log(
          `Bloco ${idx} - Day offset: ${dayOffset}, Start: ${start}, End: ${end}, IsConflict: ${isConflict}`
        );
      ...
      if (hasRoomConflict) {
        ...
        console.log("estass",isEditingEvent)
        // 3️⃣ Se estamos a editar removemos do buffer e do calendário
        if (isEditingEvent) {
          ...
          console.log("entrou", blockId)
          const startHour = new Date(selectedEvent.start)
            .toTimeString()
            .slice(0, 8);
          const endHour = new Date(selectedEvent.end)
            .toTimeString()
            .slice(0, 8);
          ...
          // 🗑️ Marcar para delete no backend se for original
          if (blockId) {
            setDeletedEvents((prev) => [...prev, blockId]);
          }

          // ❌ Remover do state para desaparecer do calendário
          setEvents((prev) =>
            prev.filter((e) => e.extendedProps.blockId !== blockId)
          );
      ...
      socket.once("respostaConflitosProfessor", ({ blocos }) => {
        ...
        const conflitosProfessor = blocos.map((bloco, idx) => {
          ...
          console.log(
            `Professor bloco ${idx} - Day offset: ${dayOffset}, Start: ${start}, End: ${end}, IsConflict: ${isConflict}`
          );
  ...
  // 1️⃣ Mostra mensagem de erro
  setMessage({
    text: "Este professor já tem aulas nesse horário!",
    type: "danger",
  });
  // 2️⃣ Desselcciona o slot de seleção
  calendarRef.current.getApi().unselect();
  // 3️⃣ Fecha o modal
  setShowRoomModal(false);

  // 4️⃣ Se estamos em edição, remove do buffer e do calendário
  if (isEditingEvent) {
    const { blockId, room: bufRoom, professor } = selectedEvent.extendedProps;
    const startHour = new Date(selectedEvent.start)
      .toTimeString()
      .slice(0, 8);
    const endHour = new Date(selectedEvent.end)
    ...
    // 🗑️ Marca para deletar no backend, se for bloco original
    if (blockId) {
      setDeletedEvents((prev) => [...prev, blockId]);
    }

    // ❌ Remove do estado para desaparecer do calendário
    setEvents((prev) =>
      prev.filter((e) => e.extendedProps.blockId !== blockId)
    );
  }
  ...
  return;
        ...
        socket.once("respostaBuffer", (resposta) => {
          if (resposta.status === "ok") {
            ...
            const eventKey =
              selectedEvent.extendedProps.blockId || selectedEvent.id;
            ...
            if (selectedEvent.isNew) {
              ...
              setMessage({
                text: "Nova aula adicionada com sucesso!",
                type: "success",
              });
            } else {
              ...
              setEvents((prevEvents) =>
                prevEvents.map((event) =>
                  ...
                  selectedEvent.extendedProps.blockId
                    ? {
                        ...event,
                        title: `${selectedCourse.name} - ${roomName} - ${selectedCourse.professor}`,
                        extendedProps: {
  ...
  const handleEventRemove = () => {
    ...
    if (
      selectedEvent.extendedProps.blockId &&
      selectedEvent.extendedProps.isOriginal
    ) {
    ...
    setEvents(events.filter((e) => String(e.id) !== String(selectedEvent.id)));
    setShowRoomModal(false);
  ...
  const saveSchedule = async () => {
    ...
    const newEvents = events.filter(
      (event) => event.isNew || event.extendedProps.isNew
    );
    ...
    const updatedEvents = existingEvents.filter((event) => {
      const original = originalEvents.find(
        (orig) => orig.extendedProps.blockId === event.extendedProps.blockId
      );
      ...
      return (
        ...
        original.extendedProps.classroomId !==
    ...
    try {
      ...
      for (const event of updatedEvents) {
        ...
        const updateData = {
          subjectId: event.extendedProps.subjectId,
          classroomId:
            roomUpdates.current[event.extendedProps.blockId || event.id]
              ?.classroomId ||
        ...
        try {
          ...
          console.log(
            "Successfully updated block with blockId:",
            event.extendedProps.blockId
          );
        } catch (error) {
          ...
          <Card className="mb-4 card">
            ...
            <Card.Body>
              <ol className="ps-3">
                ...
                <li className="mb-2">
                  Selecione uma cadeira para adicionar nova aula
                </li>
                <li className="mb-2">
                  Clique e arraste no calendário para criar aula
                </li>
                <li className="mb-2">
              ...
              <FullCalendar
                ...
                eventContent={({ event, timeText }) => (
                  <div className="d-flex justify-content-between align-items-center w-100">
                    ...
                    {!event.classNames?.includes("evento-conflito") &&
                      !event.classNames?.includes("evento-ocupado") && (
                        <FaEdit
              ...
              <Card className="mb-4 card">
                <Card.Body>
                  <Row>
                    <Col md={9}>
                      <Alert variant="info">
                        Clique numa aula para editá-la ou selecione uma cadeira
                        e clique no calendário para adicionar nova aula.
                      </Alert>
                    </Col>
                    <Col md={3} className="d-flex align-items-center">
                      <Button className="w-100 button" onClick={saveSchedule}>
                        Guardar Alterações
                      </Button>
                    </Col>
                  </Row>
